#include <iostream>
#include <deque> // deque használatához szükséges header
...
int main(int argc, char const *argv[])
{
    ...
    /*
     std::deque<Player> playersDeque; // deque létrehozása amiben Player objektumpéldányokat tudunk tárolni
     
     playersDeque.push_back(p1);
     playersDeque.push_back(p2);
     playersDeque.push_back(p3);
     
     // ha így adjuk át akkor minden egyes Player-ből egy másolat fog készülni
     // ami bekerül a deque-be, így ha szeretnénk módosítani pl. a p1-nek X és Y
     // koordinátáit (p1.setPlayerPosition(9,6);) az a deque-en belüli p1 objektumpéldányra
     // nem fog vonatkozni, mivel az csak egy másolata lesz a p1-nek. annak módosításához a
     // deque-en belüli indexével kell hivatkoznunk rá. (playersVector[0].setPlayerPosition(9,6);)
     // egy jobb megoldás lenne, ha Player-re mutató pointereket tárolnánk és a Player-eknek a
     // referenciáját adnánk át
     */
    
    std::deque<Player*> playersDeque; // deque létrehozása amiben Player-re mutató pointerek fogunk tárolni
    ...
    for (int i = 0; i < playersDeque.size(); ++i)
        std::cout << "Player neve: " << (*playersDeque[i]).getPlayerName() << ", X és Y pozíciója: "
        << (*playersDeque[i]).getXPosition() << ", " << (*playersDeque[i]).getYPosition() << std::endl;
    
    p1.setPlayerPosition(9,6); // most, ha az eredeti példányon módosítunk akkor a deque-ben lévő is módosul
    // ugyanis nem egy másolata van a playersDeque-ben, csak egy mutató az eredeti p1-re
